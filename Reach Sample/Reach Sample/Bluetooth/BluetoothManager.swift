...
class BluetoothManager: NSObject {
    ...
    fileprivate func connectToDevice(_ device: BluetoothDevice) -> Promise<Void> {
        ...
        after(seconds: 10).done(on: queue) {
            ...
            self.connected[device.id] = nil
            seal.reject(BluetoothError.couldNotConnect)
            self.centralManager.cancelPeripheralConnection(device.peripheral)
        }
        
        queue.async {
            self.connected[device.id] = (promise, seal)
...
extension BluetoothManager: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        for i in 0..<scannedDevices.count {
            if scannedDevices[i].id == scannedDevice.id {
                deviceExists = true
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        guard let device = scannedDevices.first(where: { $0.id == peripheral.identifier }) else { return }
        print("Connected to device: \(device.name ?? device.id.uuidString)")
        ...
        self.connected[device.id]?.resolver.fulfill_()
        self.connected[device.id] = nil
    }
    
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        print("Failed to connect to device: \(peripheral.name ?? "Nil")")
        guard let device = self.scannedDevices.first(where: { $0.id == peripheral.identifier }) else { return }
        
        self.stateDelegates.forEach({ $0.failedToConnectToDevice(device) })
        self.connected[device.id]?.resolver.reject(BluetoothError.couldNotConnect)
        self.connected[device.id] = nil
    }
    
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        guard let device = self.scannedDevices.first(where: { $0.id == peripheral.identifier }) else { return }
        print("Disconnected from device \(device.name ?? device.id.uuidString)")
        
        device.stateChanged(.disconnected)
        self.stateDelegates.forEach({ $0.didDisconnectFromDevice(device) })
        self.connected[device.id]?.resolver.reject(BluetoothError.connectionCancelled)
        self.connected[device.id] = nil
    }
}
...
class BluetoothDevice: NSObject {
    ...
    func readValue(from characteristic: CBCharacteristic) -> Promise<Data> {
        ...
        after(seconds: 5).done {
            guard promise.isPending, self.readMap[self.id.uuidString]?.first != nil else { return }
            self.readMap[self.id.uuidString]?.removeFirst()
    ...
    func writeValue(_ value: Data, characteristic: CBCharacteristic) -> Promise<Void> {
        ...
        after(seconds: 5).done {
            guard promise.isPending, self.writeMap[self.id.uuidString]?.first != nil else { return }
            self.writeMap[self.id.uuidString]?.removeFirst()
    ...
    func updateDevice(with device: BluetoothDevice) {
        guard device.id == self.id else { return }
...
extension BluetoothDevice {
    ...
    private func getAdvertisementInfo() -> [String : Any] {
        var advertisementJson = [String : Any]()
        advertisementData.forEach {
            if let parsed = BluetoothManager.shared.parseAdvertisementData(key: $0.key, data: advertisementData), let value = parsed.value {
    ...
    private func getCharacteristicInfo(for service: CBService) -> [[String : Any]] {
        var characteristicsJson: [[String : Any]] = []
        
        for characteristic in services[service] ?? [] {
            var characteristicJson: [String : Any] = [
            ...
            if let name = characteristic.name {
                characteristicJson["name"] = name
            }
            
            if let value = characteristic.value {
                let decoded = String(data: value, encoding: .utf8)
                let encoding = decoded == nil ? "hex" : "utf-8"
                characteristicJson["encoding"] = encoding
                characteristicJson["value"] = decoded ?? value.map { String(format: "%02hhx", $0) }.joined()
            }
            
            characteristicsJson.append(characteristicJson)
        }
